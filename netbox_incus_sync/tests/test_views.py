...
class IncusHostViewTestCase(TestCase):
    ...
    def test_incushost_detail_view(self):
        url = reverse('plugins:netbox_incus_sync:incushost', kwargs={'pk': self.incus_host.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_incushost_edit_view(self):
        url = reverse('plugins:netbox_incus_sync:incushost_edit', kwargs={'pk': self.incus_host.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_incushost_delete_view(self):
        url = reverse('plugins:netbox_incus_sync:incushost_delete', kwargs={'pk': self.incus_host.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_incushost_clear_cache_view(self):
        self.incus_host.update_working_url('https://working.local')
        url = reverse('plugins:netbox_incus_sync:incushost_clear_cache', kwargs={'pk': self.incus_host.pk})
    ...
    def test_sync_host_redirects(self):
        url = reverse('plugins:netbox_incus_sync:sync_host', kwargs={'pk': self.incus_host.pk})
        response = self.client.get(url)
