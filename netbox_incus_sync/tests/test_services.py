...
class InstanceCreationTests(IncusSyncTestCase):
    ...
    def test_create_container(self):
        ...
        service.setup()
        vm, created, _ = service.sync_instance(
            CONTAINER_BASIC, self.cluster, self.incus_host, project="default"
        )
    ...
    def test_stopped_is_offline(self):
        service = InstanceSyncService(logger=MagicMock())
        service.setup()
        vm, _, _ = service.sync_instance(
            CONTAINER_STOPPED, self.cluster, self.incus_host
        )
...
class InstanceMetadataTests(IncusSyncTestCase):
    ...
    def _sync(self, data):
        service = InstanceSyncService(logger=MagicMock())
        service.setup()
        return service.sync_instance(
            data, self.cluster, self.incus_host, project="default"
        )
...
class InstanceRenameTests(IncusSyncTestCase):
    ...
    def test_rename_by_uuid(self):
        service = InstanceSyncService(logger=MagicMock())
        service.setup()
        vm1, _, _ = service.sync_instance(
            CONTAINER_BASIC, self.cluster, self.incus_host
        )
        vm2, created, updated = service.sync_instance(
            CONTAINER_RENAMED, self.cluster, self.incus_host
        )
        ...
        self.assertFalse(
            VirtualMachine.objects.filter(name="test-container-01").exists()
        )
...
class InstanceUpdateTests(IncusSyncTestCase):
    ...
    def test_status_change(self):
        ...
        vm, created, _ = service.sync_instance(
            {**CONTAINER_BASIC, "status": "Stopped"}, None, self.incus_host
        )
        self.assertFalse(created)
        self.assertEqual(vm.status, "offline")
    ...
    def test_no_duplicates(self):
        ...
        service.sync_instance(CONTAINER_BASIC, None, self.incus_host)
        self.assertEqual(
            VirtualMachine.objects.filter(name="test-container-01").count(), 1
        )
...
class InstanceDeletionTests(IncusSyncTestCase):
    ...
    def _make_managed_vm(self, name, uuid):
        ...
        tag, _ = Tag.objects.get_or_create(
            slug="incus-managed", defaults={"name": "Managed by Incus Sync"}
        )
        vm.tags.add(tag)
        return vm
    ...
    def test_gone_vm_deleted(self):
        ...
        service.setup()
        self.assertEqual(
            service.handle_deletions(self.cluster, self.incus_host, set()), 1
        )
    ...
    def test_present_vm_kept(self):
        ...
        service.setup()
        self.assertEqual(
            service.handle_deletions(self.cluster, self.incus_host, {"keep-uuid-0000"}),
            0,
        )
...
class NetworkSyncTests(IncusSyncTestCase):
    ...
    def test_creates_interface(self):
        ...
        self.assertTrue(
            VMInterface.objects.filter(virtual_machine=vm, name="eth0").exists()
        )

    def test_loopback_skipped(self):
        vm, _, _ = self._sync_net(CONTAINER_BASIC)
        self.assertFalse(
            VMInterface.objects.filter(virtual_machine=vm, name="lo").exists()
        )
    ...
    def test_stale_cleaned(self):
        ...
        NetworkSyncService(logger=MagicMock()).sync_instance_network(
            vm, CONTAINER_BASIC, MagicMock()
        )
        self.assertFalse(
            VMInterface.objects.filter(virtual_machine=vm, name="old-iface").exists()
        )
...
class DiskSyncTests(IncusSyncTestCase):
    ...
    def test_root_disk(self):
        ...
        self.assertTrue(
            VirtualDisk.objects.filter(virtual_machine=vm, name="root").exists()
        )

    def test_root_disk_size(self):
        vm, _ = self._sync_disks(CONTAINER_BASIC)
        self.assertEqual(
            VirtualDisk.objects.get(virtual_machine=vm, name="root").size, 10240
        )
    ...
    def test_pool_cf(self):
        vm, _ = self._sync_disks(CONTAINER_BASIC)
        self.assertEqual(
            VirtualDisk.objects.get(
                virtual_machine=vm, name="root"
            ).custom_field_data.get("incus_storage_pool"),
            "default",
        )
    ...
    def test_volume_source_cf(self):
        ...
        self.assertEqual(
            VirtualDisk.objects.get(
                virtual_machine=vm, name="data"
            ).custom_field_data.get("incus_volume_source"),
            "db-data",
        )
    ...
    def test_stale_cleaned(self):
        ...
        DiskSyncService(logger=MagicMock()).sync_instance_disks(
            vm, CONTAINER_BASIC, _make_disk_client()
        )
        self.assertFalse(
            VirtualDisk.objects.filter(virtual_machine=vm, name="old-disk").exists()
        )
...
class ConfigContextSyncTests(IncusSyncTestCase):
    ...
    def test_creates_local_context(self):
        ...
        updated, created = service.sync_instance_config_context(
            vm, CONTAINER_BASIC, self.incus_host
        )
        self.assertTrue(created)
        vm.refresh_from_db()
    ...
    def test_resync_not_recreated(self):
        ...
        _, created = service.sync_instance_config_context(
            vm, CONTAINER_BASIC, self.incus_host
        )
        self.assertFalse(created)
