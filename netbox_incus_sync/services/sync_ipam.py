...
class IpamSyncService:
    ...
    def managed_tag(self):
        ...
        if self._managed_tag is None:
            self._managed_tag, _ = Tag.objects.get_or_create(
                slug="incus-managed", defaults={"name": "Incus Managed"}
            )
    ...
    def _sync_prefix(
        self, cidr, network_name, network_type, description, config, host, family="ipv4"
    ):
        """
        ...
        try:
            ...
            # Build description
            prefix_description = f"Incus network: {network_name} ({network_type})"
            if description:
            ...
            if existing:
                ...
                if updated:
                    existing.save()
                    self.log(
                        "debug", f"    Prefix updated: {prefix_cidr} ({network_name})"
                    )
                else:
                    self.log(
                        "debug", f"    Prefix exists: {prefix_cidr} ({network_name})"
                    )
            ...
            self.log(
                "warning", f"    Invalid CIDR '{cidr}' for network {network_name}: {e}"
            )
            return None, False
    ...
    def sync_device_vlan(self, device_config, host):
        ...
        network_name = device_config.get("network", "") or device_config.get(
            "parent", ""
        )
        vlan_name = (
            f"incus-{network_name}-vlan{vid}" if network_name else f"incus-vlan{vid}"
        )
    ...
    def _sync_vlan(self, vid, name, network_name, host):
        ...
        if not existing:
            ...
            existing = VLAN.objects.filter(vid=vid, tags=self.managed_tag).first()
        ...
        if any_existing:
            self.log(
                "debug", f"    Using existing VLAN: {any_existing.name} (VID {vid})"
            )
    ...
    def assign_vlan_to_interface(self, interface, device_config, host):
        ...
        if updated:
            ...
            self.log(
                "info",
                f"    Interface {interface.name}: VLAN {vlan_obj.vid} assigned (access mode)",
            )
