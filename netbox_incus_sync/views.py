    ...
    def get_extra_context(self, request, instance):
        ...
        for vm in managed_vms:
            ...
            vm_list.append(
                {
                    "vm": vm,
                    "instance_type": vm.custom_field_data.get(
...
class IncusSyncHostView(View):
    ...
    def get(self, request, pk):
        ...
        if not host.enabled:
            messages.warning(
                request, f"Host '{host.name}' is disabled. Enable it first."
            )
            return redirect("plugins:netbox_incus_sync:incushost", pk=pk)

        job = SyncIncusJob.enqueue(host_ids=[host.pk])
        messages.success(
            request, f"Synchronization started for '{host.name}' (Job #{job.pk})"
        )
    ...
    def post(self, request):
        ...
        messages.success(
            request,
            f"Synchronization started for {hosts.count()} host(s): {host_names} (Job #{job.pk})",
        )
        return redirect("plugins:netbox_incus_sync:incushost_list")
...
class IncusVMScreenshotView(View):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            return self._respond(
                request, vm, False, f"Incus host '{host_name}' not found or disabled."
            )
        ...
        try:
            ...
            ImageAttachment.objects.filter(
                object_type=vm_ct, object_id=vm.pk, name__startswith="incus-screenshot-"
            ).delete()
            ...
            return self._respond(
                request, vm, False, f"Error capturing screenshot: {str(e)}"
            )
            ...
            for url in urls:
                ...
                try:
                    ...
                    if host.client_cert_path and host.client_key_path:
                        test_session.cert = (
                            host.client_cert_path,
                            host.client_key_path,
                        )
